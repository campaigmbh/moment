...
test('diff across DST', function(assert) {
    ...
    assert.equal(b.diff(a, 'milliseconds', true), 12 * 60 * 60 * 1000, 'ms diff across DST');
    assert.equal(b.diff(a, 'seconds', true), 12 * 60 * 60, 'second diff across DST');
    assert.equal(b.diff(a, 'minutes', true), 12 * 60, 'minute diff across DST');
    assert.equal(b.diff(a, 'hours', true), 12, 'hour diff across DST');
    assert.equal(b.diff(a, 'days', true), (12 - dst.diff) / 24, 'day diff across DST');
    equal(assert, b.diff(a, 'weeks', true), (12 - dst.diff) / 24 / 7, 'week diff across DST');
    assert.ok(0.95 / (2 * 31) < b.diff(a, 'months', true), 'month diff across DST, lower bound');
    assert.ok(b.diff(a, 'month', true) < 1.05 / (2 * 28), 'month diff across DST, upper bound');
    assert.ok(0.95 / (2 * 31 * 12) < b.diff(a, 'years', true), 'year diff across DST, lower bound');
    assert.ok(b.diff(a, 'year', true) < 1.05 / (2 * 28 * 12), 'year diff across DST, upper bound');
    ...
    assert.equal(b.diff(a, 'seconds', true), (12 + dst.diff) * 60 * 60, 'second diff across DST');
    assert.equal(b.diff(a, 'minutes', true), (12 + dst.diff) * 60, 'minute diff across DST');
    assert.equal(b.diff(a, 'hours', true), 12 + dst.diff, 'hour diff across DST');
    assert.equal(b.diff(a, 'days', true), 12 / 24, 'day diff across DST');
    equal(assert, b.diff(a, 'weeks', true), 12 / 24 / 7, 'week diff across DST');
    assert.ok(0.95 / (2 * 31) < b.diff(a, 'months', true), 'month diff across DST, lower bound');
    assert.ok(b.diff(a, 'month', true) < 1.05 / (2 * 28), 'month diff across DST, upper bound');
    assert.ok(0.95 / (2 * 31 * 12) < b.diff(a, 'years', true), 'year diff across DST, lower bound');
    assert.ok(b.diff(a, 'year', true) < 1.05 / (2 * 28 * 12), 'year diff across DST, upper bound');
...
test('month diffs', function(assert) {
    ...
    assert.equal(
        moment([2019, 0, 31]).diff([2019, 1, 28], 'months'),
        -1,
        'Jan 31 to Feb 28 should be 1 month',
    );
    assert.ok(
    ...
    // Generate all months for this year. Day is 10 because all months had this day
    for (let index = 1; index <= 12; index++) {
        const date = `2019-${index < 10 ? '0' + index : index}-10`;
    ...
    // Get all differences from all combinations
    const diffs = [];

    let monthIndex = 1;

    for (let currentMonth of months) {
        // Loops through all 66 combinations of diffs
        for (let index = monthIndex; index < MONTH_COUNT; index++) {
            const testingMonth = months[index];

            const start_diff = testingMonth.start.diff(currentMonth.start, 'months', null, true);

            const end_diff = testingMonth.end.diff(currentMonth.end, 'months', null, true);

            const negative_start_diff = Math.abs(
                currentMonth.start.diff(testingMonth.start, 'months', null, true),
            );

            const negative_end_diff = Math.abs(
                currentMonth.end.diff(testingMonth.end, 'months', null, true),
            );
    ...
    for (let diff of diffs) {
        // Loops through all the fields in the differences to make sure they are all the same

        // Store first value in memory. If at least one of the values is different from the
        // one stored, then it is wrongly calculated
        let value = false;

        for (let field in diff) {
            const fieldValue = diff[field];
...
test('exact month diffs', function(assert) {
    ...
    for (m1 = 0; m1 < 12; ++m1) {
        for (m2 = m1; m2 < 12; ++m2) {
            assert.equal(
                moment([2013, m2, 15]).diff(moment([2013, m1, 15]), 'months', true),
                m2 - m1,
...
test('negative zero', function(assert) {
    ...
    assert.ok(
        !isNegative(moment([2012, 0, 1]).diff(moment([2012, 0, 1]), 'quarters')),
    ...
    assert.ok(
        !isNegative(moment([2012, 0, 1]).diff(moment([2012, 0, 1, 0, 1]), 'hours')),
        'hour diff on same hour is zero, not -0',
    );
    assert.ok(
        !isNegative(moment([2012, 0, 1]).diff(moment([2012, 0, 1, 0, 0, 1]), 'minutes')),
        'minute diff on same minute is zero, not -0',
    );
    assert.ok(
        !isNegative(moment([2012, 0, 1]).diff(moment([2012, 0, 1, 0, 0, 0, 1]), 'seconds')),
